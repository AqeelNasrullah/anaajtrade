...
@section('title')
    <title>Create Medicine Trader - {{ config('app.name') }}</title>
@endsection
...
@section('breadcrumbs')
    {{ Breadcrumbs::render('create_medicine_traders') }}
@endsection

@section('content')
    <section class="container-fluid py-3">
        <div class="w-50 form-area mx-auto">
            <h1 class="text-success mb-3 fw-900 text-center">Add Medicine Trader <br> <span class="text-urdu-kasheeda">ادویات کا تاجر شامل کریں</span></h1>
            @include('components.error')
            <form action="{{ route('medicineTraders.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                ...
                <div>
                    <button type="submit" class="btn btn-success float-right">Add Medicine Trader</button>
                    <br class="clear">
                </div>
            </form>
        </div>
