...
@section('title')
    <title>View Medicine Record for {{ $record->profile->name }} - {{ config('app.name') }}</title>
@endsection
...
@section('breadcrumbs')
    {{ Breadcrumbs::render('view_medicine_records', $record) }}
@endsection
    ...
    <section class="container-fluid py-3">
        <div class="w-50 mx-auto">
            <div class="print mb-3">
                ...
                <form action="{{ route('medicineRecord.destroy', base64_encode(($record->id * 123456789) / 12098)) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger float-right delete-slip mr-2"><i class="fas fa-trash-alt"></i> Delete</button>
                </form>
                <a href="{{ route('medicineRecord.index') }}" class="btn btn-outline-danger float-right mr-2"><i class="fas fa-times"></i> Close</a>
                <br class="clear">
            </div>
            ...
            <div class="slip">
                <div class="slip-inner mb-3" style="border-bottom: 1px dashed grey;">
                    <h3 class="text-center fw-700 mb-3">Medicine Record / <span class="text-urdu-kasheeda">دوائی کا ریکارڈ</span></h3>
                    <div class="row">
                        <div class="col-md-6">
                    ...
                    <hr class="mt-0">
                    <div class="row">
                        <p class="col-md-6 mb-1"><strong>Quantity:</strong> {{ $record->quantity }}</p>
                        <p class="col-md-6 mb-1"><strong>Medicine Type:</strong> {{ $record->medicineType->type }}</p>
                        <p class="col-md-6 mb-1"><strong>Medicine:</strong> {{ $record->medicineType->name }}</p>
