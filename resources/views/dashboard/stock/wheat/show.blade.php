    ...
    <section class="container-fluid py-3">
        <div class="slip">
            <div class="mb-3 print">
                ...
                <form action="{{ route('wheatStock.destroy', base64_encode(($stock->id * 123456789) / 12098)) }}" method="post" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-danger delete-stock float-right mr-2" type="submit"><i class="fas fa-trash-alt"></i> Delete</button>
                </form>
                <a href="{{ route('wheatStock.index') }}" class="btn btn-outline-danger float-right mr-2"><i class="fas fa-times"></i> Close</a>
                <br class="clear">
            </div>
            ...
            <div class="slip-inner">
                ...
                <div class="row">
                    ...
                    <p class="col-6 mb-1 text-right">Commission ({{ $stock->commission }}%):</p>
                    <p class="col-6 mb-0">Rs {{ (($stock->commission / 100) * (($stock->num_of_sack * $stock->weight_per_sack) / 40) * $stock->price) }} /-</p>
                    <p class="col-6 mb-0 text-right">Total Price (after commission):</p>
                    <p class="col-6">Rs {{ ((($stock->num_of_sack * $stock->weight_per_sack) / 40) * $stock->price) - (($stock->commission / 100) * (($stock->num_of_sack * $stock->weight_per_sack) / 40) * $stock->price) }} /-</p>
                    <p class="col-6 mb-0">Signature:</p>
                </div>
            </div>
            @else
