...
@section('breadcrumbs')
    {{ Breadcrumbs::render('edit_rice_stock', $stock) }}
@endsection

@section('content')
    <section class="container-fluid py-3">
        <h1 class="text-success fw-900 text-center mb-3">Update Rice Stock / <span class="text-urdu-kasheeda">چاول کا اسٹاک تبدیل کریں</span></h1>
        <div style="max-width:700px;margin:0px auto;">
            @include('components.error')
            <form action="{{ route('riceStock.update', base64_encode(($stock->id * 123456789) / 12098)) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                            ...
                            <select name="type" id="type" class="form-control">
                                ...
                                @if ($types->count() > 0)
                                    @foreach ($types as $type)
                                        <option value="{{ $type->id }}" {{ $stock->rice_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                ...
                <div>
                    ...
                    <a href="{{ route('riceStock.index') }}" class="btn btn-outline-danger float-right mr-2">Close</a>
                    <br class="clear">
                </div>
            </form>
        </div>
