...
@section('breadcrumbs')
    {{ Breadcrumbs::render('edit_fertilizer_traders', $trader) }}
@endsection

@section('content')
    <section class="container-fluid py-3">
        <div class="w-50 form-area mx-auto">
            <h1 class="text-center text-success mb-3 fw-900">Edit Fertilizer Trader / <span class="text-urdu-kasheeda">کھاد کا تاجر تبدیل کریں</span></h1>
            @include('components.error')
            @if ($trader)
            <form action="{{ route('fertilizerTraders.update', base64_encode(($trader->id * 123456789) / 12098)) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <div>
                    <button type="submit" class="btn btn-success float-right">Update Fertilizer Trader</button>
                    <a href="{{ route('fertilizerTraders.index') }}" class="btn btn-outline-danger float-right mr-2">Close</a>
                    <br class="clear">
                </div>
            </form>
            @else
                <p class="alert alert-danger w-50 mx-auto font-italic text-center">No fertilizer trader to show.</p>
            @endif
        </div>
    </section>
@endsection
